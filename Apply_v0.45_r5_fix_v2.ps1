...
if (-not $modules.Contains('src\/visual-effects\.js\?v=0\.45-r5')) { throw 'modules test cache expectation could not be updated to r5' }
...
$test = $test.Replace('src\/visual-effects\.js\?v=0\.45-r3', 'src\/visual-effects\.js\?v=0\.45-r5')
$test = $test.Replace('/intensity=appearance\.y\*variation\*visible/', '/lively=smoothstep\(\.30,\.62,appearance\.x\)/')
if (-not $test.Contains('lively=smoothstep')) { throw 'v045 tiny-star test expectation could not be updated' }
if (-not $test.Contains('src\/visual-effects\.js\?v=0\.45-r5')) { throw 'v045 cache expectation could not be updated to r5' }
...
Write-Host 'r5 repair v2 applied on top of known-good r4 HEAD.' -ForegroundColor Green
