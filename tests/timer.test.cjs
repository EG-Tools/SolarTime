...
test('timer provides exclusive sound choices, alarm actions and a zen-safe shutdown countdown',()=>{
 ...
 assert.match(html,/id="shutdown-helper-install-dialog"/);assert.match(html,/id="shutdown-helper-install-yes"/);assert.match(code,/state\.helperInstallToken&&\(!state\.helperInstallDeadline\|\|state\.helperInstallDeadline>now\(\)\)\)notify\(t\('helperDownloadStarted'\)\);else showInstallConfirm\(\)/);assert.match(code,/state\.helperConfirmed=true;state\.helperProgress=100;state\.helperEnabled=true/);
 ...
 assert.match(html,/id="shutdown-helper-remove"[^>]+disabled/);assert.match(html,/id="shutdown-helper-remove-dialog"/);assert.match(code,/shutdownBridge\?\.uninstall\?\.\(\)/);
 ...
 assert.match(code,/remaining>0&&remaining<=10/);assert.match(code,/shutdownBridge\?\.cancel\(\)/);
});
...
test('native shutdown bridge is Windows desktop only and clamps shutdown.exe seconds',()=>{
 ...
 assert.match(helper,/echo 설치가 완료되었습니다\./);assert.match(helper,/Start-Sleep -Seconds 1/);assert.match(helper,/SOLARTIME_INSTALLER_DELETE/);assert.match(helper,/Start-Sleep -Seconds 2; Remove-Item -LiteralPath \$env:SOLARTIME_INSTALLER_DELETE/);
 assert.match(helper,/Zone\.Identifier/);assert.match(helper,/install-complete\?token=/);assert.match(helper,/install=\(\[a-f0-9\]\{32\}\)/);assert.match(helper,/Start-Sleep -Seconds 30/);
...
test('Windows helper is an explicit Cloudflare opt-in and is excluded from the static site graph',()=>{
 ...
 assert.match(controller,/shutdownBridge\?\.uninstall\?\.\(\)/);assert.match(controller,/if\(hadScheduledShutdown\)shutdownBridge\?\.cancel\(\)/);assert.match(read('styles.css'),/\.toast\.card-surface\{/);
});
