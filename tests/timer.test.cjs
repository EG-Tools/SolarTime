...
test('native shutdown bridge is Windows desktop only and clamps shutdown.exe seconds',()=>{
 ...
 assert.match(helper,/Zone\.Identifier/);assert.match(helper,/install-complete\?token=/);assert.match(helper,/install=\(\[a-f0-9\]\{32\}\)/);assert.match(helper,/SolarTimeShutdownCleanup_/);assert.match(helper,/WScript\.Sleep 5000/);assert.match(helper,/SolarTimeShutdownLauncher\.vbs/);assert.match(helper,/SolarTimeShutdownInstall\.id/);assert.match(helper,/If current = expected Then/);assert.ok(helper.indexOf('shell.Run "reg.exe delete')>helper.indexOf('If current = expected Then'));assert.match(helper,/expression\.Pattern = "\^solartime-timer:/);assert.match(helper,/shell\.Run command, 0, False/);assert.match(helper,/wscript\.exe" \/\/B \/\/Nologo/);assert.doesNotMatch(helper,/start "" powershell\.exe/);assert.doesNotMatch(helper,/Start-Sleep -Seconds 30/);
