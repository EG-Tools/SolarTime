...
test('Locale requests use the current revision so eclipse labels bypass old cached copy',()=>{
  assert.match(read('index.html'),/src\/language-data\.js\?v=0\.51-r2/);
});
...
test('Direct file launches overlay current eclipse copy on older public locale bundles',async()=>{
  ...
  for(const file of files){
    const code=path.basename(file,'.json'),window={SolarModules:{}},location={protocol:'file:',href:'file:///D:/_Program/SolarTime/index.html'},document={currentScript:{src:'file:///D:/_Program/SolarTime/src/language-data.js?v=0.51-r2'}};
