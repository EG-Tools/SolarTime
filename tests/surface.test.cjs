...
test('Both source and offline build load surface before renderer; release identity is consistent',()=>{
 ...
 assert.ok(html.indexOf('src/surface.js')<html.indexOf('src/renderer.js'));assert.match(build,/'assets','astro','surface','sky','renderer','app'/);
 assert.ok(html.includes('Life User <span>/</span> v0.06'));
 assert.match(app,/version:'0\.06'/);assert.equal(JSON.parse(read('package.json')).version,'0.0.6');
 assert.ok(!html.includes('EG TOOLS'));assert.ok(html.includes('Life User / Solar Time v0.06 /')); // Historical release comparisons are allowed in help.
});
