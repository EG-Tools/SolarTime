...
function fixture(){
 const temp=fs.mkdtempSync(path.join(os.tmpdir(),'solar-v016-'));
 for(const dir of ['src','assets','tools'])fs.mkdirSync(path.join(temp,dir));
 for(const file of ['index.html','styles-v016.css','package.json','tools/build.cjs','tools/pack_assets.cjs','src/app.js','src/renderer.js','src/sky.js','src/materials.js','src/sky-asset.js','assets/universe.webp'])fs.copyFileSync(path.join(root,file),path.join(temp,file));
...
test('build embeds revised sky once and all script tags, leaving website index unchanged',()=>{
 ...
 const html=fs.readFileSync(path.join(temp,'dist/Solar-Time_v0.16.html'),'utf8');assert.doesNotMatch(html,/<script\b[^>]*\bsrc=/i);assert.doesNotMatch(html,/styles-v016\.css/);assert.doesNotMatch(html,/src\/sky-asset\.js/);
...
test('build fails clearly if an original planet asset is missing, without publishing output',()=>{
 const temp=fixture();try{fs.unlinkSync(path.join(temp,'assets/earth.webp'));const result=build(temp);assert.notEqual(result.status,0);assert.match(result.stderr,/Missing original asset/);assert.ok(!fs.existsSync(path.join(temp,'dist/Solar-Time_v0.16.html')));}finally{fs.rmSync(temp,{recursive:true,force:true});}
});
