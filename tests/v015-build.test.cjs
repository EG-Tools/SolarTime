...
test('build embeds revised sky once and all script tags, leaving website index unchanged',()=>{
 ...
 const html=fs.readFileSync(path.join(temp,'dist/Solar-Time_v0.16.html'),'utf8');assert.doesNotMatch(html,/<script\b[^>]*\bsrc=/i);assert.doesNotMatch(html,/src\/sky-asset\.js/);
