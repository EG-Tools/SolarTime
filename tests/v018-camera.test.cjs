...
test('rotation icons point in the same direction as their labels',()=>{const h=read('index.html');const left=h.match(/id="rotate-left"[\s\S]*?<\/button>/)[0],right=h.match(/id="rotate-right"[\s\S]*?<\/button>/)[0];assert.doesNotMatch(left,/transform="translate\(24 0\) scale\(-1 1\)"/);assert.match(right,/transform="translate\(24 0\) scale\(-1 1\)"/);});
