...
test('overview -> focus starts without changing the displayed camera',()=>{
 const r=fixture(),before=plain(r.camera);assert.equal(r.animateFocus('earth',100),true);assert.deepEqual(plain(r.camera),before);assert.equal('bridge' in r.cameraTween,false);
...
test('home transition blends directly to exact home without a bridge zoom',()=>{
 const r=fixture();r.camera={...r.camera,focus:'earth',zoom:64};r.animateHome(0);near(r.camera.zoom,64);r.advanceCamera(550);assert.equal(r.camera.focus,null);assert.ok(r.camera.zoom>1&&r.camera.zoom<64);r.advanceCamera(1100);assert.equal(r.camera.focus,null);near(r.camera.zoom,1);near(r.camera.azimuth,25*DEG);
});
test('different closeups use one continuous direct transition',()=>{
 const r=fixture();r.camera={...r.camera,focus:'earth',zoom:64};r.animateFocus('moon',0);assert.equal('bridge' in r.cameraTween,false);
 const half=r.cameraTween.duration/2;r.advanceCamera(half);assert.equal(r.camera.focus,'moon');assert.ok(r.camera.zoom>1);r.advanceCamera(r.cameraTween?.duration||1100);assert.equal(r.camera.focus,'moon');
});
...
test('wheel targets accumulate without losing deltas during interpolation',()=>{
 ...
 near(r.cameraTween.to.zoom,1.2**3);assert.equal(r.cameraTween.to.focus,null);assert.ok(r.camera.zoom<r.cameraTween.to.zoom);r.advanceCamera(170);near(r.camera.zoom,1.2**3);
});
test('pan and drag targets clamp but do not snap',()=>{
 const r=fixture();r.smoothCamera({panX:2,panY:-2,elevation:3},0);near(r.camera.panX,0);near(r.cameraTween.to.panX,.4);near(r.cameraTween.to.panY,-.4);near(r.cameraTween.to.elevation,Math.PI/2);r.advanceCamera(130);near(r.camera.panX,.4);
});
...
test('feature view uses the same tween rather than immediate focus + orbit setters',()=>{
 const r=fixture(),before=plain(r.camera);assert.ok(r.animateFeature('earth',37,126,10000,0));assert.deepEqual(plain(r.camera),before);assert.equal(r.cameraTween.to.focus,'earth');assert.equal(r.cameraTween.to.panX,0);assert.equal(r.cameraTween.to.panY,0);assert.ok(r.cameraTween.to.zoom>=224);assert.ok(Number.isFinite(r.cameraTween.to.azimuth));
});
...
test('numeric preset data schema and old localStorage keys remain compatible',()=>{
 const r=fixture();assert.ok(Renderer.validCamera(r.camera));const app=fs.readFileSync(path.join(root,'src/app.js'),'utf8');assert.match(app,/eg\.solar-time\.v0\.01/);assert.match(app,/solar-time\.camera-presets\.v1/);assert.match(app,/version:'0\.24'/);
});
