...
test('numeric preset data schema and old localStorage keys remain compatible',()=>{
 const r=fixture();assert.ok(Renderer.validCamera(r.camera));const app=fs.readFileSync(path.join(root,'src/app.js'),'utf8');assert.match(app,/eg\.solar-time\.v0\.01/);assert.match(app,/solar-time\.camera-presets\.v1/);assert.match(app,/version:'0\.23'/);
});
