...
image=png()
def load(browser,root,size,standalone=False,locale='ko-KR'):
 context=browser.new_context(viewport=dict(width=size[0],height=size[1]),locale=locale,timezone_id='Asia/Seoul',reduced_motion='reduce',has_touch=standalone)
...
def suite(browser,root,size,installed):
 ...
 check(page.locator('#release-notes-version').inner_text()=='v0.51',tag+' visible release version')
 check(page.locator('#release-notes-list li').count()==6 and 'JPL' in page.locator('#release-notes-list').inner_text(),tag+' visible English SG release note')
 page.locator('#release-notes-older').click();check(page.locator('#release-notes-version').inner_text()=='v0.50' and 'Auto Language' in page.locator('#release-notes-list').inner_text(),tag+' previous release navigation')
 ...
 for _ in range(3):page.locator('#release-notes-newer').click()
 check(page.locator('#release-notes-version').inner_text()=='v0.51',tag+' current release navigation')
 ...
 for country,lang,zone,label,city,lat,lon in [('nl','nl-NL','Europe/Amsterdam','NETHERLANDS','Amsterdam',52+22/60,4.9),('be','nl-BE','Europe/Brussels','BELGIUM','Brussel',50+50/60,4+20/60)]:
  ...
  check(page.locator('#release-notes-version').inner_text()=='v0.51' and 'JPL' in page.locator('#release-notes-list').inner_text(),tag+' '+country+' current Dutch release')
  page.locator('#release-notes-older').click();page.locator('#release-notes-older').click()
  check('Willekeurige rotatie' in page.locator('#release-notes-list').inner_text(),tag+' '+country+' Dutch release history')
  page.locator('#release-notes-newer').click();page.locator('#release-notes-newer').click()
  page.locator('#help-dialog .close-button').first.click()
