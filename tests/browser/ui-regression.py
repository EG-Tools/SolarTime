...
def suite(browser,root,size,installed):
 ...
 check(page.locator('#release-notes-version').inner_text()=='v0.48',tag+' visible release version')
 check(page.locator('#release-notes-list li').count()==6 and '250×' in page.locator('#release-notes-list').inner_text(),tag+' visible English SG release note')
 ...
 page.locator('#release-notes-newer').click();check(page.locator('#release-notes-version').inner_text()=='v0.48',tag+' current release navigation')
