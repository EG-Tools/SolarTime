...
def suite(browser,root,size,installed):
 ...
 check(page.locator('#release-notes-list li').count()==5 and 'clock-size' in page.locator('#release-notes-list').inner_text().lower(),tag+' visible English SG release note')
 page.locator('#release-notes-older').click();check(page.locator('#release-notes-version').inner_text()=='v0.53' and 'cookie' in page.locator('#release-notes-list').inner_text().lower(),tag+' previous release navigation')
 ...
 for country,lang,zone,label,city,lat,lon in [('nl','nl-NL','Europe/Amsterdam','NETHERLANDS','Amsterdam',52+22/60,4.9),('be','nl-BE','Europe/Brussels','BELGIUM','Brussel',50+50/60,4+20/60)]:
  ...
  for _ in range(5):page.locator('#release-notes-older').click()
