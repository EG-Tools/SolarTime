...
def suite(browser,root,size,installed):
 ...
 check(page.locator('#release-notes-version').inner_text()=='v0.54',tag+' visible release version')
 ...
 check(page.locator('#release-notes-version').inner_text()=='v0.54',tag+' current release navigation')
 page.locator('#help-dialog .close-button').first.click()
 ...
 for country,lang,zone,label,city,lat,lon in [('nl','nl-NL','Europe/Amsterdam','NETHERLANDS','Amsterdam',52+22/60,4.9),('be','nl-BE','Europe/Brussels','BELGIUM','Brussel',50+50/60,4+20/60)]:
  ...
  check(page.locator('#release-notes-version').inner_text()=='v0.54' and 'klokgrootte' in page.locator('#release-notes-list').inner_text(),tag+' '+country+' current Dutch release')
