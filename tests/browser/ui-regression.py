...
def suite(browser,root,size,installed):
 ...
 check(page.locator('#release-notes-version').inner_text()=='v0.55',tag+' visible release version')
 check(page.locator('#release-notes-list li').count()==4 and 'Microsoft Clarity' in page.locator('#release-notes-list').inner_text(),tag+' visible English SG release note')
 page.locator('#release-notes-older').click();check(page.locator('#release-notes-version').inner_text()=='v0.54' and 'clock-size' in page.locator('#release-notes-list').inner_text().lower(),tag+' previous release navigation')
 ...
 check(page.locator('#release-notes-version').inner_text()=='v0.55',tag+' current release navigation')
 page.locator('#help-dialog .close-button').first.click()
 ...
 for country,lang,zone,label,city,lat,lon in [('nl','nl-NL','Europe/Amsterdam','NETHERLANDS','Amsterdam',52+22/60,4.9),('be','nl-BE','Europe/Brussels','BELGIUM','Brussel',50+50/60,4+20/60)]:
  ...
  check(page.locator('#release-notes-version').inner_text()=='v0.55' and 'Microsoft Clarity' in page.locator('#release-notes-list').inner_text(),tag+' '+country+' current Dutch release')
  for _ in range(6):page.locator('#release-notes-older').click()
  check('Willekeurige rotatie' in page.locator('#release-notes-list').inner_text(),tag+' '+country+' Dutch release history')
  for _ in range(6):page.locator('#release-notes-newer').click()
  page.locator('#help-dialog .close-button').first.click()
