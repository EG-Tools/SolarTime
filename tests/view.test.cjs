...
test('Legacy 64x bookmarks retain the same screen radius for ALL bodies and viewports',()=>{
...
test('Close-view command targets the same readable screen radius, including Moon and Pluto',()=>{
 for(const body of all){const r=renderer();r.focusBody(body.id);assert.equal(r.camera.focus,body.id);near(r.bodyRadiusAtZoom(body),Math.min(r.w,r.h)*.25);assert.ok(r.camera.zoom>3&&r.camera.zoom<64);}
});
