"""v0.11 release gate: offline startup, controls, sphere geometry, materials & lifecycle.
...
report={'version':'0.11','checks':[],'measurements':{},'limitations':[]}
...
with sync_playwright() as tool:
 ...
 started=time.perf_counter();page.set_content((ROOT/'dist/Solar-Time_v0.11.html').read_text(),wait_until='load')
 ...
 check('Identity and footer contact',page.locator('.signature').inner_text().startswith('Life User') and 'v0.11' in page.locator('.signature').inner_text())
 ...
 page.locator('#universe').focus();page.keyboard.press('h');page.wait_for_timeout(200);page.screenshot(path=str(OUT/'zen-v0.11.png'));check('Pointer activity reveals only home and mode actions',set(page.locator('button:visible').evaluate_all('(els)=>els.map(e=>e.id)'))=={'fit-view','show-ui'});page.keyboard.press('Escape')
 for target in ['earth','moon','jupiter','saturn']:
  ...
  page.screenshot(path=str(OUT/(target+'-v0.11.png')))
 ...
 for width,height in [(390,844),(320,568)]:
  ...
  page.locator('#settings-button').click();check(f'{width}px settings contains contact and offline description','Life User / Solar Time v0.11 /' in page.locator('.settings-credit').inner_text() and '인터넷 없이' in page.locator('.settings-note').inner_text())
  page.locator('#settings-close').click()
  page.screenshot(path=str(OUT/f'mobile-{width}-v0.11.png'))
...
(ROOT/'docs/release-browser-v0.11.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
