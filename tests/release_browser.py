"""v0.06 release gate: offline startup, controls, sphere geometry, materials & lifecycle.
...
report={'version':'0.06','checks':[],'measurements':{},'limitations':[]}
...
with sync_playwright() as tool:
 ...
 try:file_page.goto((ROOT/'dist/Solar-Time_v0.06.html').as_uri(),wait_until='load',timeout=5000)
 except Exception as e:report['limitations'].append('Direct file navigation: '+str(e).splitlines()[0])
 file_page.close()
 ...
 started=time.perf_counter();page.set_content((ROOT/'dist/Solar-Time_v0.06.html').read_text(),wait_until='load')
 page.wait_for_function('window.SolarTime?.version==="0.06"',timeout=15000)
 ...
 check('Identity and footer contact',page.locator('.signature').inner_text().startswith('Life User') and 'v0.06' in page.locator('.signature').inner_text())
 ...
 page.keyboard.press('h');check('Same home button remains visible in viewing mode',page.locator('#fit-view').is_visible());page.locator('#fit-view').click()
 page.wait_for_timeout(1900);check('Viewing-mode cursor hides while home stays visible',page.evaluate('getComputedStyle(document.body).cursor')=='none' and page.locator('#fit-view').is_visible());page.keyboard.press('Escape')
 ...
 page.screenshot(path=str(OUT/'overview-v0.06.png'))
 for target in ['earth','moon','jupiter','saturn']:
  ...
  page.screenshot(path=str(OUT/(target+'-v0.06.png')))
 ...
 for width,height in [(390,844),(320,568)]:
  ...
  page.locator('#settings-button').click();check(f'{width}px settings contains contact and offline description','Life User / Solar Time v0.06 /' in page.locator('.settings-credit').inner_text() and '인터넷 없이' in page.locator('.settings-note').inner_text())
  page.locator('#settings-close').click()
  page.screenshot(path=str(OUT/f'mobile-{width}-v0.06.png'))
...
(ROOT/'docs/release-browser-v0.06.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
