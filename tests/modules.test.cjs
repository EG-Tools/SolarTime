...
test('runtime concerns load as modules before the application coordinator',()=>{
 ...
 assert.ok(html.includes('src/surface.js?v=0.55-r1'));
 assert.ok(html.includes('src/renderer.js?v=0.55-r1'));
 assert.ok(html.includes('src/performance.js?v=0.55-r1'));
  ...
  assert.ok(html.includes('src/'+name+'.js?v='+ (name==='ui-runtime'?'0.55-r1':'0.45')));
    assert.ok(html.indexOf('src/'+name+'.js')<html.indexOf('src/app.js'),name);
  }
 assert.ok(html.includes('src/language-data.js?v=0.55-r1'));
...
test('phone layout shares an explicit standalone-aware flag with the sky renderer',()=>{
 ...
 assert.ok(read('index.html').includes('src/runtime-optimizations.css?v=0.55-'+JSON.parse(read('version.json')).revision));
 assert.ok(read('index.html').includes('src/page-runtime.js?v=0.55-'+JSON.parse(read('version.json')).revision));
});
