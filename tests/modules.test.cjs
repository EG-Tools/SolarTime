...
test('runtime concerns load as modules before the application coordinator',()=>{
  ...
  assert.ok(html.includes('src/surface.js?v=0.53-r2'));
  assert.ok(html.includes('src/renderer.js?v=0.53-r2'));
  assert.ok(html.includes('src/performance.js?v=0.53-r2'));
  ...
  for(const name of ['preferences','ui-runtime','music-player']){
    assert.ok(html.includes('src/'+name+'.js?v='+ (name==='ui-runtime'?'0.53-r2':'0.45')));
    assert.ok(html.indexOf('src/'+name+'.js')<html.indexOf('src/app.js'),name);
  }
  assert.ok(html.includes('src/language-data.js?v=0.53-r2'));
