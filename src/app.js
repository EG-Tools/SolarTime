  ...
  async function init() {
    try {
      ...
      const CURRENT_RELEASE_ITEMS=Object.freeze({
        kor:Object.freeze(['별 밀도 기본값을 200%로 조정하고, 구 전체에 더 균일하게 퍼지는 저편향 분포로 별 간격을 넓혔습니다.','0~400% 슬라이더는 최대 17,600개의 GPU 별 풀에서 실제 그리는 개수만 바꾸며 별 밀도 항목의 구분선도 제거했습니다.','태양 표면은 움직임 속도는 유지하면서 무늬를 더 작고 촘촘하게 만들고 변형 폭을 낮춰 굵은 출렁임을 줄였습니다.','목성에는 위도별 가스 띠가 서로 다른 속도로 미세하게 흐르는 전용 대기 셰이더를 추가했습니다.']),
        en:Object.freeze(['Star density now defaults to 200%, using a low-discrepancy full-sphere distribution with wider, more even spacing.','The 0–400% slider changes only the draw count inside a fixed 17,600-star GPU pool, and its divider has been removed.','The Sun keeps the same motion speed but uses much finer spatial detail and lower warp amplitude to avoid broad wobble.','Jupiter now has a dedicated atmospheric shader with subtle differential flow between latitude bands.']),
        chn:Object.freeze(['星星密度默认调整为 200%，并改为覆盖整个球面的低偏差分布，让间距更均匀、更开阔。','0–400% 滑块只改变固定 17,600 颗 GPU 星星池的实际绘制数量，同时移除了该选项的分隔线。','太阳保持原有运动速度，但纹理更细密、变形幅度更小，减少粗大的晃动感。','木星加入专用大气着色器，使不同纬度的气体条带以略有不同的速度细微流动。']),
        jpn:Object.freeze(['星の密度の初期値を 200% にし、球全体へ均等に広がる低偏差分布で間隔を広げました。','0～400% スライダーは固定 17,600 個の GPU 星プールから描画数だけを変え、項目の区切り線も削除しました。','太陽は動く速度を保ったまま模様を細かくし、変形量を抑えて大きなうねりを減らしました。','木星には緯度ごとのガス帯が少しずつ異なる速度で流れる専用大気シェーダーを追加しました。']),
        hi:Object.freeze(['Star density का default 200% किया गया है और पूरे sphere में अधिक समान, कम-cluster वाली low-discrepancy distribution लागू की गई है।','0–400% slider fixed 17,600-star GPU pool में केवल draw count बदलता है और इस option की divider line हटाई गई है।','Sun की गति वही है, लेकिन pattern अधिक महीन और warp कम है ताकि बड़े wobble कम दिखें।','Jupiter के लिए latitude bands में हल्का differential gas flow दिखाने वाला अलग atmospheric shader जोड़ा गया है।']),
        es:Object.freeze(['La densidad de estrellas pasa a 200% por defecto y usa una distribución de baja discrepancia más uniforme sobre toda la esfera.','El control 0–400% solo cambia el número dibujado dentro de un pool GPU fijo de 17.600 estrellas y se elimina su separador.','El Sol mantiene la misma velocidad, pero con detalle espacial más fino y menor amplitud para evitar ondulaciones gruesas.','Júpiter recibe un shader atmosférico propio con flujo diferencial sutil entre bandas de latitud.']),
        de:Object.freeze(['Die Sterndichte startet nun bei 200% und nutzt eine gleichmäßigere Low-Discrepancy-Verteilung über die gesamte Kugel.','Der 0–400%-Regler ändert nur die Draw-Anzahl in einem festen GPU-Pool mit 17.600 Sternen; die Trennlinie entfällt.','Die Sonne behält ihre Bewegungsgeschwindigkeit, nutzt aber feinere Strukturen und kleinere Verformungen gegen grobes Wobbeln.','Jupiter erhält einen eigenen Atmosphären-Shader mit subtil unterschiedlicher Strömung der Breitenbänder.']),
        fr:Object.freeze(['La densité d’étoiles passe à 200% par défaut avec une distribution à faible discrépance plus uniforme sur toute la sphère.','Le réglage 0–400% ne change que le nombre dessiné dans un pool GPU fixe de 17 600 étoiles, sans ligne de séparation.','Le Soleil conserve la même vitesse mais avec des motifs plus fins et une déformation réduite pour éviter les grosses ondulations.','Jupiter reçoit un shader atmosphérique dédié avec un léger flux différentiel entre les bandes de latitude.'])
      });
