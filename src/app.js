/* Solar Time v0.45 r2 — calmer randomized stars, softer Sun motion, and stock Jupiter shading. */
(function () {
  'use strict';
  ...
  const FACTORY_OPTIONS=Object.freeze({actualScale:false,overviewOrbitGap:86,orbitBrightness:.5,starDensity:2,dollyZoom:false,labels:true,avoidLabels:false,twinkle:true,activity:true,pluto:true,moon:true,skyMotion:true,comets:true,quality:'auto'});
  ...
  async function init() {
    try {
      ...
      const CURRENT_RELEASE_ITEMS=Object.freeze({
        kor:Object.freeze(['별의 위치·크기·밝기·색상을 서로 독립적으로 랜덤화하고 황색 별 비중을 줄였습니다.','대부분의 별은 안정된 밝기를 유지하고 일부만 5~25초 범위로 느리고 불규칙하게 반짝이며, 아주 소수만 5~10초 동안 잠시 사라지거나 드물게 십자 광채를 냅니다.','태양 표면의 촘촘한 흐름 구조는 유지하면서 현재 효과에서 약 5% 더 낮춰 움직임을 조금 더 차분하게 조정했습니다.','목성의 실험적 제트·난류·대적점 셰이더를 제거하고 업그레이드 전 기본 가스행성 셰이더로 완전히 복원했습니다.']),
        en:Object.freeze(['Star position, size, brightness and colour are randomized independently, with yellow stars made less common.','Most stars remain steady; only some twinkle slowly and irregularly over 5–25 seconds, while a very small minority briefly disappear for 5–10 seconds or show rare cross flares.','The Sun keeps its fine flow structure while the current motion strength is reduced by another 5% for a calmer result.','Jupiter’s experimental jets, turbulence and Great Red Spot shader are removed, fully restoring the original gas-giant shader.']),
        chn:Object.freeze(['星星的位置、大小、亮度和颜色彼此独立随机，并减少黄色星星的比例。','大多数星星保持稳定亮度，只有部分星星以 5～25 秒的周期缓慢且不规则地闪烁；极少数会短暂隐藏 5～10 秒或偶尔出现十字光芒。','太阳保留细密流动结构，并在当前效果基础上再降低约 5% 的运动强度。','移除木星实验性的喷流、湍流和大红斑着色效果，完全恢复升级前的基础气态巨行星着色器。']),
        jpn:Object.freeze(['星の位置・大きさ・明るさ・色を独立してランダム化し、黄色い星の割合を減らしました。','大部分の星は安定した明るさを保ち、一部だけが 5～25 秒の範囲でゆっくり不規則に瞬きます。ごく少数だけが 5～10 秒消えたり、まれに十字光を見せます。','太陽は細かな流れを保ちながら、現在の表面運動をさらに約 5% 弱めました。','木星の実験的なジェット・乱流・大赤斑シェーダーを削除し、変更前の標準ガス惑星シェーダーへ完全に戻しました。']),
        hi:Object.freeze(['तारों की position, size, brightness और colour अलग-अलग random हैं और yellow stars का अनुपात कम किया गया है।','अधिकांश तारे स्थिर रहते हैं; कुछ ही 5–25 सेकंड में धीमे और अनियमित रूप से twinkle करते हैं, और बहुत कम तारे 5–10 सेकंड के लिए छिपते या कभी-कभार cross flare दिखाते हैं।','Sun की fine flow structure बनी रहती है, लेकिन मौजूदा motion strength को और लगभग 5% कम किया गया है।','Jupiter के experimental jets, turbulence और Great Red Spot shader हटाकर पुराने मूल gas-giant shader को पूरी तरह वापस किया गया है।']),
        es:Object.freeze(['La posición, tamaño, brillo y color de las estrellas se aleatorizan por separado, y las estrellas amarillas son menos frecuentes.','La mayoría mantiene un brillo estable; solo algunas centellean de forma lenta e irregular entre 5 y 25 segundos, y una fracción mínima desaparece 5–10 segundos o muestra raros destellos en cruz.','El Sol conserva su estructura fina, pero la intensidad del movimiento actual se reduce otro 5% para resultar más calmada.','Se eliminan los jets, turbulencias y el shader experimental de la Gran Mancha Roja de Júpiter, restaurando por completo el shader original de gigante gaseoso.']),
        de:Object.freeze(['Position, Größe, Helligkeit und Farbe der Sterne werden unabhängig zufällig verteilt; gelbe Sterne treten seltener auf.','Die meisten Sterne bleiben nahezu konstant; nur einige funkeln langsam und unregelmäßig über 5–25 Sekunden, während sehr wenige 5–10 Sekunden verschwinden oder selten Kreuzstrahlen zeigen.','Die feine Sonnenstruktur bleibt erhalten, die aktuelle Bewegungsstärke wird jedoch um weitere etwa 5% reduziert.','Die experimentellen Jupiter-Jets, Turbulenzen und der Shader für den Großen Roten Fleck werden entfernt; der ursprüngliche Gasriesen-Shader ist vollständig wiederhergestellt.']),
        fr:Object.freeze(['La position, la taille, la luminosité et la couleur des étoiles sont randomisées indépendamment, avec moins d’étoiles jaunes.','La plupart restent presque stables ; seules certaines scintillent lentement et irrégulièrement sur 5 à 25 secondes, tandis qu’une très petite minorité disparaît 5 à 10 secondes ou produit rarement une lueur en croix.','Le Soleil conserve sa structure fine, mais l’intensité actuelle du mouvement est encore réduite d’environ 5%.','Les jets, turbulences et le shader expérimental de la Grande Tache rouge de Jupiter sont supprimés, rétablissant complètement le shader original de géante gazeuse.'])
      });
      ...
      const PREVIOUS_RELEASE_ITEMS=Object.freeze({
        ...
        jpn:Object.freeze(['星の密度は初期値 200% のまま、位置・大きさ・明るさ・色・瞬き周期を独立してランダム化し、規則的な並びをなくしました。','低彩度の赤・白・青白・黄の星を混ぜ、明るい星のごく一部だけにまれな十字光を加えました。','太陽は速度を変えず、細かなスケールの変形を強めて表面の小さな動きを見えやすくしました。','木星は緯度別ジェットと渦を強め、大赤斑周辺だけがゆっくり回る局所渦を追加しました。']),
        hi:Object.freeze(['Star density का default 200% रखते हुए position, size, brightness, colour और twinkle timing को अलग-अलग random किया गया है ताकि नियमित pattern न दिखे।','हल्के red, white, blue-white और yellow रंग मिलते हैं, और केवल कुछ चमकीले तारों पर कभी-कभार cross flare दिखाई देता है।','Sun की गति वही रखते हुए fine-scale warp मजबूत किया गया है ताकि छोटे surface motions साफ दिखें।','Jupiter में latitude jets और eddies मजबूत किए गए हैं तथा Great Red Spot के पास धीमा local vortex जोड़ा गया है।']),
        es:Object.freeze(['La densidad por defecto sigue en 200%, pero posición, tamaño, brillo, color y periodo de parpadeo se aleatorizan de forma independiente.','Se mezclan estrellas rojas, blancas, blanco-azuladas y amarillas con poca saturación, y solo unas pocas brillantes muestran destellos en cruz.','El Sol conserva la velocidad pero aumenta la deformación fina para que el movimiento de pequeña escala sea claramente visible.','Júpiter refuerza sus chorros y remolinos e incorpora una rotación local lenta alrededor de la Gran Mancha Roja.']),
