...
(function () {
  ...
  const COPY={
    ...
    en:{
      ...
      representation:'Type',star:'Star',systemCenter:'Center of the solar system',sunNote:'Bright and dark plasma detail drifts subtly across the rotating surface. Its motion and surrounding shine are visual effects, not live solar observation data.',earthOrbitPeriod:'Orbit around Earth',brightSide:'Illuminated side seen from Earth · approximate',approximately:'about {value}',moonNote:'{phase} · mean orbit position aligned to a JPL reference time · mean distance fixed',jupiterOrbitPeriod:'Orbit around Jupiter',jupiterDistance:'Mean distance to Jupiter',europaNote:'Current orbital position aligned to a JPL reference time · mean distance fixed; long-term minute changes are omitted.',orbitPeriod:'Orbital period',sunDistance:'Distance from Sun · approximate',plutoNote:'Fixed mean orbit · not for precise position prediction.',earthNote:'{region} · {city} is in {dayNight} / solar altitude about {altitude}° · simple approximation',day:'daylight',night:'night',auNote:'1 AU is the mean distance between Earth and the Sun. Sizes and distances are rescaled for the display.',koreaView:'View {region} · day/night',stormView:'View the Great Red Spot',
  ...
  async function init() {
    try {
      ...
      let timezone='local',showSeconds=false,hourCycle='24',language='kor',zen=false,raf=0,lastFrame=0,effectTime=0,lastWallKey='',lastUi=0,disposed=false;
      ...
      function updateBody(ms) {
        ...
        const period=hours>=48?quantity(Math.abs(b.spin).toFixed(2),'dayUnit'):`${quantity(hours,'hourUnit')} ${quantity(minutes,'minuteUnit')} ${quantity(seconds,'secondUnit')}`;
        const screenSeconds=Math.abs(b.spin)*86400/clock.rate;
        const playback=clock.paused?t('paused'):clock.live?t('realTime'):screenSeconds<1?t('timesPerSecond',{value:(1/screenSeconds).toFixed(2)}):t('secondsPerTurn',{value:screenSeconds.toFixed(screenSeconds<100?1:0)});
        $('body-spin').textContent=t('rotationSummary',{period,retrograde:b.spin<0?t('retrograde'):'' ,playback});
        if(b.id==='sun') {
          ...
          $('body-note').textContent=t('sunNote');
        } else if(b.id==='moon') {
          ...
          $('body-note').textContent=t('moonNote',{phase:phaseCopy(phase.name)});
        } else if(b.id==='europa') {
          ...
          $('body-note').textContent=t('europaNote');
        } else {
          ...
          const region=activeRegion(),altitude=A.siteSun(ms,region.latitude,region.longitude).altitude;$('body-note').textContent=b.id==='pluto'?t('plutoNote'):b.id==='earth'?t('earthNote',{region:region.region,city:region.city,dayNight:t(altitude>=0?'day':'night'),altitude:altitude.toFixed(1)}):t('auNote');
        }
      }
