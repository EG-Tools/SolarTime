/* Solar Time v0.56 — app implementation owner. */
(function () {
  'use strict';
...
,
  ...
  async function init() {
    try {
      ...
      timerController=Modules.TimerController.create({document,UI,Preferences,translate:t,notify:toast,shutdownBridge:Modules.WindowsShutdown.create(document),onOpen:()=>{settings(false);closeBody();},shouldKeepOpen:event=>$('help-dialog').open||$('help-button').contains(event.target)});
      translateStatic();
      renderer.resize();
      ...
      function loadReleaseNotes(){
        ...
        return UI.loadScript('src/release-notes.js?v=0.56-r1','SolarReleaseNotes').then(api=>{if(!releaseNotesApi){releaseNotesApi=api;releaseNotesNavigator=api.createReleaseNotesNavigator();}return releaseNotesApi;});
      }
