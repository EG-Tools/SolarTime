/* Solar Time v0.45 r3 — randomized star layouts, calmer twinkle, softer Sun motion, and stock Jupiter shading. */
(function () {
  'use strict';
  ...
  async function init() {
    try {
      ...
      const CURRENT_RELEASE_ITEMS=Object.freeze({
        kor:Object.freeze(['별 밀도 기본값을 100%로 조정하고 새로고침·재실행·초기화 때마다 별 위치를 새롭게 랜덤 배치하며, 크기·밝기·색상도 서로 독립적으로 랜덤화하고 황색 별 비중을 줄였습니다.','대부분의 별은 안정된 밝기를 유지하고 일부만 5~25초 범위로 느리고 불규칙하게 반짝이며, 아주 소수만 5~10초 동안 잠시 사라지거나 드물게 십자 광채를 냅니다.','태양 표면의 촘촘한 흐름 구조는 유지하면서 현재 효과에서 약 5% 더 낮춰 움직임을 조금 더 차분하게 조정했습니다.','목성의 실험적 제트·난류·대적점 셰이더를 제거하고 업그레이드 전 기본 가스행성 셰이더로 완전히 복원했습니다.']),
        en:Object.freeze(['Star density now defaults to 100%; positions are randomized again on reload, restart and reset, while size, brightness and colour remain independently randomized.','Most stars remain steady; only some twinkle slowly and irregularly over 5–25 seconds, while a very small minority briefly disappear for 5–10 seconds or show rare cross flares.','The Sun keeps its fine flow structure while the current motion strength is reduced by another 5% for a calmer result.','Jupiter’s experimental jets, turbulence and Great Red Spot shader are removed, fully restoring the original gas-giant shader.']),
        chn:Object.freeze(['星星密度默认调整为 100%；页面刷新、重新启动或重置时都会重新随机分布星星位置，大小、亮度和颜色也保持彼此独立随机。','大多数星星保持稳定亮度，只有部分星星以 5～25 秒的周期缓慢且不规则地闪烁；极少数会短暂隐藏 5～10 秒或偶尔出现十字光芒。','太阳保留细密流动结构，并在当前效果基础上再降低约 5% 的运动强度。','移除木星实验性的喷流、湍流和大红斑着色效果，完全恢复升级前的基础气态巨行星着色器。']),
        jpn:Object.freeze(['星の密度の初期値を 100% にし、再読み込み・再起動・初期化のたびに位置を再ランダム化し、大きさ・明るさ・色も独立してランダム化します。','大部分の星は安定した明るさを保ち、一部だけが 5～25 秒の範囲でゆっくり不規則に瞬きます。ごく少数だけが 5～10 秒消えたり、まれに十字光を見せます。','太陽は細かな流れを保ちながら、現在の表面運動をさらに約 5% 弱めました。','木星の実験的なジェット・乱流・大赤斑シェーダーを削除し、変更前の標準ガス惑星シェーダーへ完全に戻しました。']),
