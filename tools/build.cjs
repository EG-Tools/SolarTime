/* v0.35: the website works as supplied. This optional build makes one offline HTML. */
'use strict';
const fs=require('node:fs'),path=require('node:path');
const root=path.resolve(__dirname,'..'),read=file=>fs.readFileSync(path.join(root,file),'utf8');
const {offlineAssetScript}=require('./asset-pipeline.cjs');
...
try{
 ...
 if(version!=='0.35')throw Error('This builder requires package version 0.0.35.');
 ...
 for(const name of names){
  ...
  const code=(name==='assets'?offlineAssetScript(root):read('src/'+name+'.js')).replace(/<\/script/gi,'<\\/script');
