...
const REQUIRED=Object.freeze(['sun','mercury','venus','earth','earth-night','mars','jupiter','saturn','uranus','neptune','pluto','moon','europa','clouds']);
...
async function textureVariants(root,output,prefix,file,group='textures'){
 ...
 for(const width of widths){
  const seamBaked=group==='textures',lossless=file==='earth-night.webp',bytes=seamBaked?await seamBakedVariant(source,width,width/2,lossless):(width===metadata.width?fs.readFileSync(source):await sharp(source).resize(width,width/2,{fit:'fill',kernel:sharp.kernel.lanczos3}).webp(lossless?{lossless:true,effort:6}:{quality:88,effort:6,smartSubsample:true}).toBuffer());
...
function compactRuntimeManifest(manifest){
 const tiers=entry=>entry.tiers.map(row=>({width:row.width,path:row.path}));
 return {revision:manifest.revision,
  materials:Object.fromEntries(Object.entries(manifest.materials).map(([id,entry])=>[id,{seamBaked:!!entry.seamBaked,tiers:tiers(entry)}])),
  sky:{tiers:tiers(manifest.sky)},
  music:Object.fromEntries(Object.entries(manifest.music).map(([file,entry])=>[file,{path:entry.path}]))};
}
function runtimeScripts(root,manifest,deployment,{cdnBase=deployment.cdnBase}={}){
 ...
 const assets=prelude.replace("const local=id==='earth-night'&&location.protocol==='file:'?new URL('assets/earth-night.webp',location.href).href:'';","const local='';");
 return {assets,sky};
}
module.exports={TEXTURE_WIDTHS,SKY_FILE,REQUIRED,atomicWrite,buildMedia,compactRuntimeManifest,projectConfig,runtimeScripts,sourceFiles};
